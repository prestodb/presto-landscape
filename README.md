...
![Landscape Logo](https://presto.landscape2.io/images/left-logo.svg)

This landscape is intended as a map to explore the Presto Ecosystem, and also shows the member companies of the Presto Foundation. It is modeled after the Cloud Native Computing Foundation (CNCF) [landscape](https://landscape.cncf.io) and based on the same open-source code.

This repository contains the data files and images required to generate the [Presto Landscape](https://landscape.cncf.io). The software that generates it can be found at the [cncf/landscape2](https://github.com/cncf/landscape2) repository. Please see its [README file](https://github.com/cncf/landscape2#landscape2) for more information about how it works.
...
### Updating Presto Foundation Members

Presto Foundation Member data in this repository ( which in the [landscape.yml](landscape.yml) file are under the category `Presto Foundation Member` ) are built nightly using the [LFX Landscape Tools](https://github.com/jmertic/lfx-landscape-tools).  Any changes made directly in the data files in this repository for the above-referenced category will be overwritten.

- For `Presto Foundation Member` entries, you can make these changes in [LFX Organization Dashboard](https://docs.linuxfoundation.org/lfx/organization-dashboard/organization-profile).
...
```shell
landscape2 build --data-file landscape.yml --settings-url https://raw.githubusercontent.com/cncf/landscape2-sites/refs/heads/main/presto/settings.yml --logos-path hosted_logos --output-dir build
